...
def get_data(card, save_images=False, save_dir=None):
    ...
    try:
        username = card.find_element(by=cb.By.XPATH, value='.//span').text
    except:
        return

    try:
        handle = card.find_element(by=cb.By.XPATH, value='.//span[contains(text(), "@")]').text
    except:
        return

    try:
        postdate = card.find_element(by=cb.By.XPATH, value='.//time').get_attribute('datetime')
    except:
        return

    try:
        text = card.find_element(by=cb.By.XPATH, value='.//div[2]/div[2]/div[1]').text
    except:
        text = ""

    try:
        embeddedElement = card.find_element(by=cb.By.XPATH, value='.//div[2]/div[2]/div[2]')
        embedded = embeddedElement.text
    except:
        embedded = ""
    ...
    try:
        reply_cnt = card.find_element(by=cb.By.XPATH, value='.//div[@data-testid="reply"]').text
    except:
        reply_cnt = 0

    try:
        retweet_cnt = card.find_element(by=cb.By.XPATH, value='.//div[@data-testid="retweet"]').text
    except:
        retweet_cnt = 0

    try:
        like_cnt = card.find_element(by=cb.By.XPATH, value='.//div[@data-testid="like"]').text
    except:
        like_cnt = 0

    try:
        elements = card.find_elements(by=cb.By.XPATH, value='.//div[2]/div[2]//img[contains(@src, "https://pbs.twimg.com/")]')
    ...
    try:
        promoted = card.find_element(by=cb.By.XPATH, value='.//div[2]/div[2]/[last()]//span').text == "Promoted"
    except:
        promoted = False
    if promoted:
        return
    ...
    try:
        emoji_tags = card.find_elements(by=cb.By.XPATH, value='.//img[contains(@src, "emoji")]')
    except:
        return
    emoji_list = []
    ...
    # tweet url
    try:
        element = card.find_element(by=cb.By.XPATH, value='.//a[contains(@href, "/status/")]')
    ...
    # mention 
    try:
        links = embeddedElement.find_elements(by=cb.By.TAG_NAME, value='a')
    except:
        return
    mentions = []
    a = "dfads"
    for link in links:
        if len(link.text) > 0 and link.text[0] == '@':
            mentions.append(link.text)
    # print("mentions")
    # print(mentions)

    scrape_date = datetime.date.today().strftime("%Y-%m-%d")
    tweet = (
        scrape_date, username, handle, postdate, text, embedded, emojis, reply_cnt, retweet_cnt, like_cnt, mentions, image_links, tweet_url)
    return tweet
...
def init_driver(headless=True, proxy=None, show_images=False, option=None, firefox=False, env=None):
    ...
    if firefox:
        driver = selenium.webdriver.Firefox(options=options, executable_path=driver_path)
    else:
        capabilities = webdriver.DesiredCapabilities.CHROME.copy()
        capabilities['acceptSslCerts'] = True 
        capabilities['acceptInsecureCerts'] = True
        driver = selenium.webdriver.Chrome(options=options, executable_path=driver_path, desired_capabilities=capabilities)
...
def get_last_date_from_csv(path):
    df = pd.read_csv(path)
    return datetime.datetime.strftime(max(pd.to_datetime(df["ScrapeDate"])), '%Y-%m-%dT%H:%M:%S.000Z')
...
def keep_scroling(driver, data, writer, tweet_ids, scrolling, tweet_parsed, limit, scroll, last_position,
    ...
    while scrolling and tweet_parsed < limit:
        ...
        page_cards = driver.find_elements(by=cb.By.XPATH, value='//article[@data-testid="tweet"]')  # changed div by article
...
def check_exists_by_xpath(xpath, driver):
    timeout = 3
    try:
        driver.find_element(by=cb.By.XPATH, value=xpath)
    except exceptions.NoSuchElementException:
        return False
    return True
